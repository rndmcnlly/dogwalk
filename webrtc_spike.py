...
engineering ability. A Dog is ephemeral and task-scoped. Give each new Dog a
short pronounceable name inspired by its task. Ask the User a question only when
...
Safety mode: this local spike is read-only. Do not modify files, install
dependencies, run commands that change state, or commit unless the User grants
an ACP permission request through Walker. End with a concise plain-language
report for Walker to relay.
...
class SessionLog:
    def __init__(self) -> None:
        ...
        self.path = directory / f"{stamp}-webrtc.jsonl"
...
class AcpPack:
    ...
    def __init__(self, log: SessionLog, cwd: Path) -> None:
        self.log = log
        self.cwd = cwd
    ...
    def dispatch(self, tool: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if tool == "sic_dog":
            ...
            with self._lock:
                if name in self.dogs and self.dogs[name]["status"] == "working":
                    return {
                        "ok": False,
                        "error": f"A Dog named {name} is already working.",
                    }
                self.dogs[name] = {
                    "status": "working",
                    "task": arguments["task"],
            ...
            return {
                ...
                "name": name,
                "status": "working",
                "message": "The read-only Dog is scouting the local workspace.",
            }
        ...
        with self._lock:
            ...
            if tool == "relay_to_dog":
                if dog["status"] != "working":
                    return {
                        "ok": False,
                        "error": f"{name} is already {dog['status']} and cannot receive a follow-up.",
                    }
                return {
                    "ok": False,
                    "error": "This first ACP spike cannot redirect an active Dog. Call it off and sic a fresh Dog.",
                }
            if tool == "call_off_dog":
                if dog["status"] != "working":
                    return {
                        "ok": False,
                        "error": f"{name} is already {dog['status']} and cannot be called off.",
                    }
                future = dog["future"]
                future.cancel()
    ...
    async def _run(self, name: str, task: str) -> None:
        ...
        prompt = DOG_BRIEFING.format(name=name, task=task)
        try:
            ...
            with self._lock:
                ...
                if dog["status"] == "working":
                    ...
                    self._completed.append(
                        {"name": name, "status": "done", "report": dog["report"]}
                    )
            self.log.write("dog_finished", dog=name, status="done")
    ...
    async def request_permission(
        ...
        with self._lock:
            self._pending_decisions[decision_id] = {**event, "future": future}
    ...
    def pending_decisions(self) -> list[dict[str, Any]]:
        with self._lock:
            return [
                {key: value for key, value in decision.items() if key != "future"}
